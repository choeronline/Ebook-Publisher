...
class EpubHtml:
    def __init__(
        self, title="Title", file_name="file_name.xhtml", lang="lang", tocTitle=None
    ):
        self.title = title
...
def write_epub(title, book):
    with ZipFile(title, "w") as Zip:
        opf_content = (
            ...
            + escape(book.author)
            + "</dc:creator>\n  </metadata>\n  <manifest>"
        )  # building the content.opf file as we iterate through items
        # add default files
        ...
        for item in book.item_list:
            if type(item) is not EpubNav and type(item) is not EpubNcx:
                Zip.writestr(
                    "EPUB/" + escape(item.file_name),
                    '<?xml version="1.0" encoding="utf-8"?>\n                                        <!DOCTYPE html>\n                                        <html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" epub:prefix="z3998: http://www.daisy.org/z3998/2012/vocab/structure/#" lang="'
                    + escape(item.lang)
                    + '" xml:lang="'
                    + escape(item.lang)
                    + '">\n                                        <head>\n                                        <title>'
...
if __name__ == "__main__":
    print(
        "You have mistakenly run this file, epub.py. It is not meant to be run. It must be imported by another python file (or an implementation can be added to this __main__ section)."
    )
