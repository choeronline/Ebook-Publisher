...
class Chyoa:
    def __init__(self, url):
        ...
        if soup.find("form", attrs={"id": "immersion-form"}) is not None:
            inputs = soup.find("form", attrs={"id": "immersion-form"}).find_all(
                "input", attrs={"value": ""}
            )
            with lock:
                if Common.mt == True:
                ...
                for i in range(len(inputs)):
                    ...
                    print(
                        ...
                        + str(i)
                        + " "
                        + soup.find("label", attrs={"for": "c" + str(i)}).get_text()
                        + " ("
                        + inputs[i].get("placeholder")
            ...
            if soup.find("div", attrs={"class": "chapter-content"}).find("img"):
                with lock2:
                    for simg in soup.find(
                        ...
                        Common.urlDict[self.ogUrl][len(Common.urlDict[self.ogUrl])] = (
                            imgtemp
                        )
                self.hasimages = True
        ...
        try:
            self.questions.insert(
                0, soup.find("header", attrs={"class": "question-header"}).get_text()
            )
        ...
        if not self.backwards:
            ...
            for i in soup.find("div", attrs={"class": "question-content"}).find_all(
                "a"
            ):
                link = i.get_text()
            ...
            for u in urls:
                if Common.mt and not self.partial:
                    chapNum = int(
                        soup.find("p", attrs={"class": "meta"}).get_text().split()[1]
                    )
                    firstLinkId = None
                    threading.Thread(
    ...
    def AddNextPage(self, args):
        ...
        try:
            self.questions.append(
                soup.find("header", attrs={"class": "question-header"}).get_text()
            )
        except AttributeError:
        ...
        prevPathParts = urllib.parse.urlparse(prevLinkCheck2)[2].split(".")
        prevLinkId1 = prevPathParts[2] if len(prevPathParts) > 2 else None

        currPathParts = urllib.parse.urlparse(url)[2].split(".")
        currLinkId = currPathParts[2] if len(currPathParts) > 2 else None
    ...
    def ThreadAdd(
        ...
        #    return None
        self.Pages[self.Pages.index(url)] = Page(
            url,
            depth,
            renames,
            oldnames,
...
class Page:
    ...
    def ThreadAdd(
        ...
        # print("found issue at" + str(url))
        self.children[self.children.index(url)] = self.__class__(
            url,
            depth,
            renames,
            oldnames,
